...
func (s *jsiiProxy_SopsSyncProvider) validateAddAgeKeyFromSsmParameterParameters(param interface{}, encryptionKey awskms.IKey) error {
	...
	case string:
		// ok
	case awsssm.IStringParameter:
		// ok
	default:
		if !_jsii_.IsAnonymousProxy(param) {
			return fmt.Errorf("parameter param must be one of the allowed types: *string, awsssm.IStringParameter; received %#v (a %T)", param, param)
		}
	}

	if encryptionKey == nil {
		return fmt.Errorf("parameter encryptionKey is required, but nil was provided")
	}
