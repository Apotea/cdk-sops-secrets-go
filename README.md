...
jsii Go target, runs `jsii-pacmak`, synthesizes a stack through the result, then commits
`cdksopssecrets/`, tags it, and opens a pull request merging it into `main`. Peer dependencies are
installed at the newest version satisfying upstream's range, which is the interface surface Go
resolves consumers to.
